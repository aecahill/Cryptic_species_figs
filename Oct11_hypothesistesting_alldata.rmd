...
survey <- read.csv(file="978_CScpx_100cols_corrige_151123.csv"   , header=TRUE ) #Adjusted Nov 15 2023 to fix errors in duplicated species
...
Result: we do NOT see more diagnostic differences in sympatry than allopatry; hypothesis supported.
...
```{r}
summary(glm(CSss~HKKv3+phylum_wormsV1,data=surveymorpho,family=binomial))
drop1(glm(CSss~HKKv3+phylum_wormsV1,data=surveymorpho,family=binomial),test="Chisq")
...
```{r}
drop1(glm(CSss~HKKv3+phylum_class,data=largeclasstable,family=binomial),test="Chisq")
```
...
```{r}
drop1(glm(CSss~ferti+phylum_class,data=surveymorpho,family=binomial),test="Chisq")
```
