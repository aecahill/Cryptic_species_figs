...
levels(survey$decade)<-c("1760","1770","1780","1790","1800","1810","1820","1830","1840",
...
decadetable$Decade<-c("1760","1770","1780","1790","1800","1810","1820","1830","1840",
                      "1850","1860","1870","1880","1890","1900","1910","1920","1930","1940",
                      "1950","1960","1970","1980","1990","2000","2010","2020")

summary(lm(Percent_SL~as.numeric(Decade),data=decadetable))
...
This is significant but the trends are messy and I haven't thought them through. Clearest result is that when HKK=1000, the proportion diminishes through time, contrary to expectations.
...
for (i in 1:length(decadetable[,2])) {
  ...
  h<-(decadetable[i,1]/(decadetable[i,1]+decadetable[i,2]+decadetable[i,3]))
  percent_hard<-rbind(percent_hard,h)
}

decadetable<-as.data.frame(cbind(decadetable,percent_easy,percent_hard))
colnames(decadetable)<-c("Difficult","Easy","Normal","Percent_Easy","Percent_Hard")
...
summary(lm(Percent_Easy~as.numeric(decade),data=decadetable))
summary(lm(Percent_Hard~as.numeric(decade),data=decadetable))

#stacked bars
decadetable2<-cbind(decadetable[,1:3],decadetable[,6])
decadetable3<-pivot_longer(as.data.frame(decadetable2), cols=1:3, names_to = "Difficulty",
                       values_to = "Number", values_drop_na = FALSE)
colnames(decadetable3)<-c("Decade","Difficulty","Number_Hard")
ggplot(decadetable3,aes(x=Decade,y=Number_Hard,fill=Difficulty,))+geom_bar(position="fill",stat="identity")
ggplot(decadetable3,aes(x=Decade,y=Number_Hard,fill=Difficulty,))+geom_bar(stat="identity")

```

Again hypothesis not supported overall - no significant result with regressions. BUT looking at percent difficult shows clearer patterns (increase through time).

## In sum, when we only look at species with CS (i.e. our database), the effect of taxonomic work goes away. This is interesting since that was the strongest effect in paper 1.

## Effect of recent divergence - dispersal
# Larval type -- Planktotrophic/pelagic larvae leads to less CS/NS)?
Note1: for this I'm working with a separate surveylarv input file where I have corrected larval types to pt, lt, dd, and removed all others, including cases where there are multiple types checked. (Most of those cases are where the two CS have different larvae.) This left 574 CS with larval info.
...
Result: larvae with direct development have the highest number of CS/NS, and this is significant, but the graph doesn't make this obvious (lots of noise). Hypothesis supported.
...
Result: No difference in the number of CS per NS across HKK classes; the average for all is around 3. Hypothesis not supported.

## In sum, support for the role of recent divergence as measured by a proxy of dispersal is equivocal and depends on the proxy used.
...
# What about sediment vs rocky?
sed_hab<-cbind(survey$Hsubstrate,survey$Morpho_diff,survey$CSss)
sed_hab<-as.data.frame(na.omit(sed_hab)) #235 cases left
colnames(sed_hab)<-c("Hsubstrate","Morpho_diff","CSss")


table(sed_hab$CSss,sed_hab$Hsubstrate)
chisq.test(sed_hab$CSss,sed_hab$Hsubstrate)

sed_count<-table(sed_hab$Hsubstrate)
seddiff<-table(sed_hab$CSss,sed_hab$Hsubstrate) 
CSss_total<-seddiff[2,]/sed_count
CSsl_total<-seddiff[1,]/sed_count
sedtable<-rbind(CSss_total,CSsl_total)
sedtable #this should be the percentages for each, sym and not, columns sum to 1

surveysedtable<-pivot_longer(as.data.frame(sedtable), cols=1:3, names_to = "Substrate",
                               values_to = "Percent", values_drop_na = FALSE)
surveysedtable$CSss_type<-c("CSss","CSss","CSss","CSsl","CSsl","CSsl")
surveysedtable$Substrate<-c("Rocky","Sediment","Both","Rocky","Sediment","Both")

ggplot(as.data.frame(surveysedtable),aes(x=Substrate,y=Percent,fill=CSss_type,))+geom_bar(stat="identity")

```

Result: marginal - but in predicted direction. More CSss found in stable environments. Hypothesis supported.

# Note also that stabilizing selection predicts more CSss than CSsl, which is what we find (see above).

## In sum, we generally have support for this idea.
...
Note3: Only 294 cases are considered here, meaning that is where we have morphological data AND larval type data
...
Results: More diagnostic differences (=CSsl) at lower HKK values, both when considering 3 morpho classes AND just ss vs sl
...
These results aren't statistically significant (ANCOVA) but the graph is in line with the hypothesis. Marginal support.
...
surveyCSsymtable<-pivot_longer(as.data.frame(CSsymtable), cols=1:2, names_to = "Sympatry",
                                   values_to = "Percent", values_drop_na = FALSE)
surveyCSsymtable$CSss_type<-c("CSss","CSss","CSsl","CSsl")
...
Result: we do see more diagnostic differences in sympatry than allopatry; hypothesis supported.
...
skel_count<-table(surveymorpho$hard_skeleton)
skeldiff<-table(surveymorpho$CSss,surveymorpho$hard_skeleton) 
CSss_total<-skeldiff[2,]/skel_count
CSsl_total<-skeldiff[1,]/skel_count
skeltable<-rbind(CSss_total,CSsl_total)
skeltable #this should be the percentages for each, sym and not, columns sum to 1

surveyskeltable<-pivot_longer(as.data.frame(skeltable), cols=1:2, names_to = "Hard_skel",
                               values_to = "Percent", values_drop_na = FALSE)
surveyskeltable$CSss_type<-c("CSss","CSss","CSsl","CSsl")

ggplot(as.data.frame(surveyskeltable),aes(x=Hard_skel,y=Percent,fill=CSss_type,))+geom_bar(stat="identity")

```

Note: We do not get the same result if I just consider ss vs sl - there is no longer significance. I think this is because hard skeleton should be important for both statistical and diagnostic differences.
...
table(surveymorpho$CSss,surveymorpho$image)
chisq.test(surveymorpho$CSss,surveymorpho$image)


image_count<-table(surveymorpho$image)
imagediff<-table(surveymorpho$CSss,surveymorpho$image) 
CSss_total<-imagediff[2,]/image_count
CSsl_total<-imagediff[1,]/image_count
imagetable<-rbind(CSss_total,CSsl_total)
imagetable #this should be the percentages for each, sym and not, columns sum to 1

surveyimagetable<-pivot_longer(as.data.frame(imagetable), cols=1:2, names_to = "Image",
                               values_to = "Percent", values_drop_na = FALSE)
surveyimagetable$CSss_type<-c("CSss","CSss","CSsl","CSsl")

ggplot(as.data.frame(surveyimagetable),aes(x=Image,y=Percent,fill=CSss_type,))+geom_bar(stat="identity")
...
morphoferti_count<-table(surveymorpho$ferti)
fertidiff<-table(surveymorpho$CSss,surveymorpho$ferti) 
CSss_total<-fertidiff[2,]/morphoferti_count
CSsl_total<-fertidiff[1,]/morphoferti_count
morphofertitable<-rbind(CSss_total,CSsl_total)
morphofertitable #this should be the percentages for each, eco and not
#columns sum to 1
surveyfertitable<-pivot_longer(as.data.frame(morphofertitable), cols=1:3, names_to = "Fertilization",
                                    values_to = "Percent", values_drop_na = FALSE)
surveyfertitable$Diff_type<-c("CSss","CSss","CSss","CSsl","CSsl","CSsl")

ggplot(as.data.frame(surveyfertitable),aes(x=Fertilization,y=Percent,fill=Diff_type,))+geom_bar(stat="identity")


```

Result: hypothesis supported. Species with internal fert are more likely to have diag diffs, ie CSsl. This is also true if I remove the "both" category. Hypothesis accepted.
...
morphogen_count<-table(surveymorpho$genitals)
gendiff<-table(surveymorpho$CSss,surveymorpho$genitals) 
CSss_total<-gendiff[2,]/morphogen_count
CSsl_total<-gendiff[1,]/morphogen_count
morphogentable<-rbind(CSss_total,CSsl_total)
morphogentable #this should be the percentages for each, eco and not
#columns sum to 1
surveygentable<-pivot_longer(as.data.frame(morphogentable), cols=1:3, names_to = "Genitals",
                                    values_to = "Percent", values_drop_na = FALSE)
surveygentable$Diff_type<-c("CSss","CSss","CSss","CSsl","CSsl","CSsl")

ggplot(as.data.frame(surveygentable),aes(x=Genitals,y=Percent,fill=Diff_type,))+geom_bar(stat="identity")


```

Result: more diagnostic traits with external, hypothesis accepted. This is also true if only considering CSss and CSsl, AND when gastropods are removed (the only class with "both").

## In sum, except "image vision" we still see an importance of taxonomic characters in determining the TYPE of CS that is found (ss vs sl).
...
#Now, November 14, doing more analyses
```{r}
summary(glm(CSss~HKKv3+phylum_class,data=large2classtable,family=binomial))
drop1(glm(CSss~phylum_wormsV1,data=surveymorpho,family=binomial),test="Chisq")
summary(glm(CSss~phylum_class,data=large2classtable,family=binomial))
drop1(glm(CSss~HKKv3+phylum_class,data=surveymorpho,family=binomial),test="Chisq")


```
These show no effect of phylum on the probability of haviing CSss, BUT a strong effect of class.

What if we analyze for only large classes?
...
for (i in 1:length(surveymorpho$NameBeforeCor)) {
  ...
  large2class<-rbind(large2class,b)
  
}

large2classtable<-as.data.frame(cbind(surveymorpho,large2class))
large2classtable<-large2classtable[!is.na(large2classtable$large2class),]
```

This gave us a dataframe where only classes with AT LEAST TEN CS are considered.

Redoing analyses above:
```{r}
summary(glm(CSss~as.character(HKKv3)+phylum_class,data=large2classtable,family=binomial),test="Chisq")
```

Classes are still significant here.


I guess I need to try the other analyses but I am not sure how to interpret these things - DF sometimes goes to zero. There remains an effect of class. I don't understand what is happening here.
```{r}
drop1(glm(CSss~hard_skeleton+phylum_class,data=largeclasstable,family=binomial),test="Chisq")
```
