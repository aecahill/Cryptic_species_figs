...
survey<-filter(survey, phylum_wormsV1=="Arthropoda")
surveylarv<-filter(surveylarv, phylum_wormsV1=="Arthropoda")
...
No difference between categories - reject hypothesis. This is a different pattern than everything else so far!
...
decadetable$decade<-c("1750","1760","1770","1780","1790","1800","1810","1820","1830","1840",
                      "1850","1860","1870","1880","1890","1920","1930","1960","1970","1980","1990","2000","2010")
...
There is no trend in the percent of CSsl (diagnostic differences) through time; hypothesis not supported. However, there are significant differences between decades. Regression?
...
This is marginally significant. A bit messy. HKK=100 trends down, but so does HKK=5. Others less clear.
...
Hypothesis maybe supported? General downward trend, though the first two decades are low.

## In sum, when we only look at species with CS (i.e. our database), the effect of taxonomic work goes away. This is interesting since that was the strongest effect in paper 1. THIS IS STILL TRUE with the arthropods, though note some support for habitat access.

## Effect of recent divergence - dispersal
# Larval type -- Planktotrophic/pelagic larvae leads to less CS/NS)?
Note1: for this I'm working with a separate surveylarv input file where I have corrected larval types to pt, lt, dd, and removed all others, including cases where there are multiple types checked. (Most of those cases are where the two CS have different larvae.) This left 107 CS with larval info.
...
Hypothesis marginally supported. DD with higher CS per NS.
...
Result: Hypothesis only marginally supported. There are differences here but no clear trend - HKK=25 is clearly lowest and then higher on both ends.

## In sum, some support for the role of recent divergence as measured by a proxy of dispersal when considering only the arthropods.
...
Result: More CSss found in stable environments. Hypothesis supported.

# Note also that stabilizing selection predicts more CSss than CSsl, which is not what we find (see above).
...
Note3: Only 61 cases are considered here, meaning that is where we have morphological data AND larval type data AND Arthropod
...
Results: Very significant, but the clearest pattern is a high number of diagnostic diffs in organisms at HKK=25, with a u-shaped curve instead of a linear trend. Investigate further.

## In sum, we MAY see support for the neutral hypothesis if we use HKK as a proxy for Ne but NOT if we use reproductive investment.
...
Everyone has a hard skeleton, so no test.
...
Result: hypothesis not supported. Very small number of external fertilization, though.
...
Result: Same as above; no support.

## In sum, no support for the role of biological traits within the arthropods, but these traits are fixed or nearly so across this group of species.

## So, overall conclusions: When looking at just the arthropods, some differences from the whole dataset. No role of biological factors, but some indications that taxonomy may be important here. 