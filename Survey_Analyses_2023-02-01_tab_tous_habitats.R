...
nomsp <-read.csv2(file="File_S2.csv" ,header=T)# need to load all nominal spp data for some comparisons 
...
survey$genus <-word(survey$acceptedName_worms,1)
...
write.csv2(MissPerZone, file="Missed biological species per zone NCBI TRUE.csv")

# DO NOT FORGET: Need to reload nomsp with ALL data for other use of nomsp below:
nomsp <-read.csv2(file="File_S2.csv" ,header=T)
...
write.csv2(info, file="Missed biological species per phylum.csv")
...
write.csv2(info, file="Missed biological species per class.csv")
...
write.csv2(tab_phyla,file="Joint_Table_19phyla_670NS.csv",row.names=T) 
...
f    
 write.csv2(f,file="670NS_Fisher_chi_p_values_and_HKKv3.csv")
...
## (2) With distinct habitats: to see relationships between geo_dif (or eco_dif or morpho_dif) with habitats  did not use all habitat categories in this round below (see other loop below)
 ...
 f    
 write.csv2(f,file="670NS_Fisher_chi_p_values_Habitats.csv")
