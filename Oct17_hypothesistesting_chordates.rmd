...
survey<-filter(survey, phylum_wormsV1=="Chordata")
surveylarv<-filter(surveylarv, phylum_wormsV1=="Chordata")
...
decadetable$decade<-c("1750","1760","1770","1780","1800","1810","1820","1830","1840",
                      "1850","1860","1870","1880","1890","1900","1910","1920","1930","1940",
...
This is no longer significant. However, decrease in HKK=100 through time and an increase in HKK=10 through time. Seems opposite of predictions. 
...
So the result is not significant but the graph is very much in line with our predictions - easy species mostly described early. Not many of those, though. 

## In sum, when we only look at species with CS (i.e. our database), the effect of taxonomic work goes away. This is interesting since that was the strongest effect in paper 1. Seems to still be true with the chordates.

## Effect of recent divergence - dispersal
# Larval type -- Planktotrophic/pelagic larvae leads to less CS/NS)?
Note1: for this I'm working with a separate surveylarv input file where I have corrected larval types to pt, lt, dd, and removed all others, including cases where there are multiple types checked. (Most of those cases are where the two CS have different larvae.) This left 129 CS with larval info.
...
Hypothesis not supported. Way more species with planktotrophic larvae here.
...
Result: Hypothesis not supported; most of these species are either at HKK=10 or HKK=100.

## In sum, NO support for the role of recent divergence as measured by a proxy of dispersal when considering only chordates. Note small sample size, and that most chordates are either ascidians or bony fish.
...
Result: More CSss NOT found in stable environments. Hypothesis NOT supported. 

# Note also that stabilizing selection predicts more CSss than CSsl, which is what we find (see above). 

## In sum, we have mixed support for this idea.
...
Note3: Only 62 cases are considered here, meaning that is where we have morphological data AND larval type data AND chordates! Small sample size!

```{r}
surveylarv2<-filter(surveylarv, Morpho_diff != "NA") #make matrix where only the 
surveylarv2<-filter(surveylarv2, Repro_invest != "NA") #make matrix where only 
table(surveylarv2$Morpho_diff,surveylarv2$Repro_invest)
chisq.test(surveylarv2$Morpho_diff,surveylarv2$Repro_invest)

#and a graph

larvrepro_count<-table(surveylarv2$Repro_invest)
larvdiff<-table(surveylarv2$Morpho_diff,surveylarv2$Repro_invest) 
stat_diag<-larvdiff[3,]/larvdiff[1,]
stat_anydiff<-larvdiff[3,]/(larvdiff[3,]+larvdiff[1,])
stat_total<-larvdiff[3,]/larvrepro_count
diag_total<-larvdiff[1,]/larvrepro_count
nod_total<-larvdiff[2,]/larvrepro_count
larvreprotable<-rbind(stat_total,diag_total,nod_total)
larvreprotable #this should be the percentages for each, eco and not
#columns sum to 1

surveyreproinvesttable<-pivot_longer(as.data.frame(larvreprotable), cols=1:2, names_to = "Reproductive_Investment",
                                   values_to = "Percent", values_drop_na = FALSE)
surveyreproinvesttable$Diff_type<-c("Statistical","Statistical","Diagnostic","Diagnostic","No_Difference","No_Difference")

ggplot(as.data.frame(surveyreproinvesttable),aes(x=Reproductive_Investment,y=Percent,fill=Diff_type,))+geom_bar(stat="identity")

```
Result: significant. Hypothesis supported - lots more diagnostic differences with high reproductive investment. This is very different than at the whole dataset level!
...
Results: No longer significant, either with 3 morpho types OR when testing CSss vs sl. But, trend towards more ss cases in higher HKK, which does support the hypothesis. 

## In sum, we see support for the neutral hypothesis with BOTH reproductive investment and HKK.
...
These results aren't statistically significant (ANCOVA) but the graph is in line with the hypothesis. HOWEVER, error overlaps a lot. Marginal support.
...
Result: Not supported. Very small sample size here, just 52 cases.
...
Very significant here, but in a direction OPPOSITE what is expected. I wonder if this has to do with fish vs ascidians - are ascidians more likely to be diagnostic? Look into.
...
```
Result: results again in a direction OPPOSITE what is expected. Same values as above, because image vision and skeleton are linked traits in chordates. Basically, fish are more likely to be statistically differentiated and ascidians are more likely to be diagnostically different.
...
Result: SAME AS ABOVE - significant but in direction opposite expectations; correlated / linked with class, because ascidians are 'both'.
...
## In sum, within chordates the biological traits appear important but we need to also consider taxonomy, which is confounded here. 

## So, overall conclusions: When looking at just the chordates, many patterns have changed relative to the whole dataset. Most notably, we now have support for reproductive investment but have lost support for biological factors (and HKK?) within the class.