...
10. Ok, now larvae.
```{r}
surveylarvmorpho<-filter(survey, Morpho_diff != "NA")
...
```{r}
#Note Jule 15 2025 -- had to edit this figure to only use the large classes, not sure why. But it should work now.
# Also: I'd like to figure out how to add the phylopics within ggplot for a nicer figure
...
largeclasstable2<-largeclasstable[!is.na(largeclasstable$largeclass),]
largeclasstable3<-droplevels(largeclasstable2)
#largeclasstable2<-na.omit(largeclasstable,cols="largeclass")

#Now, we need the proportion of CSss in each large class

classtable<-table(largeclasstable3$phylum_class,largeclasstable3$CSss)
percent_ss<-c()
...
Hkk_count["HKK"][Hkk_count["HKK"] == "1"] <- "A001"
Hkk_count["HKK"][Hkk_count["HKK"] == "5"] <- "B005"
Hkk_count["HKK"][Hkk_count["HKK"] == "10"] <- "C010"
Hkk_count["HKK"][Hkk_count["HKK"] == "25"] <- "D025"
...
HKKtable2$Diff_type<-c("Statistical","Statistical","Statistical","Statistical","Statistical","Statistical","Pseudo-CS (Diagnostic)","Pseudo-CS (Diagnostic)","Pseudo-CS (Diagnostic)","Pseudo-CS (Diagnostic)","Pseudo-CS (Diagnostic)","Pseudo-CS (Diagnostic)","Undifferentiated","Undifferentiated","Undifferentiated","Undifferentiated","Undifferentiated","Undifferentiated")
HKKtable2["HKK"][HKKtable2["HKK"] == "1"] <- "1"
HKKtable2["HKK"][HKKtable2["HKK"] == "5"] <- "5"
HKKtable2["HKK"][HKKtable2["HKK"] == "10"] <- "10"
HKKtable2["HKK"][HKKtable2["HKK"] == "25"] <- "25"
HKKtable2["HKK"][HKKtable2["HKK"] == "100"] <- "100"
HKKtable2["HKK"][HKKtable2["HKK"] == "1000"] <- "1000"
HKKtable2$extent<-factor(HKKtable2$HKK,levels=c("1","5","10","25","100","1000"))

palwes<-c("black","darkgrey","lightgrey")


alldataplot<-ggplot(as.data.frame(HKKtable2),aes(x=as.factor(extent),y=Percent,fill=Diff_type,))+
  geom_bar(stat="identity")+
  scale_fill_manual(values=palwes)+
  xlab("Habitat Extent")+
  labs(fill = "Differentiation type")+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90))+
  annotate("text", x = 1, y = 0.1, label = "1", size = 4.5)+
  annotate("text", x = 2, y = 0.1, label = "20", size = 4.5)+
  annotate("text", x = 3, y = 0.1, label = "34", size = 4.5)+
  annotate("text", x = 4, y = 0.1, label = "37", size = 4.5)+
  annotate("text", x = 5, y = 0.1, label = "22", size = 4.5)+
  annotate("text", x = 6, y = 0.1, label = "11", size = 4.5)+
  ...
  annotate("text", x = 6, y = 0.5, label = "32", size = 4.5)+
  theme(panel.background = element_blank(), 
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        plot.background = element_blank(),
...
Ecograph2<-ggplot(data = surveyeco) +
  ...
  labs(x ="In Sympatry", y = stringr::str_wrap("Ecological Differences", width = 15))+
  theme_bw()+
  theme(legend.title=element_blank())+
  theme(panel.background = element_blank(), 
        panel.grid.major = element_blank(),
...
HKKtable2sed<-pivot_longer(as.data.frame(HKKtablesed), cols=1:4, names_to = "HKK",
                        values_to = "Percent", values_drop_na = FALSE)
HKKtable2sed$Diff_type<-c("Statistical","Statistical","Statistical","Statistical","Pseudo-CS (Diagnostic)","Pseudo-CS (Diagnostic)","Pseudo-CS (Diagnostic)","Pseudo-CS (Diagnostic)","Undifferentiated","Undifferentiated","Undifferentiated","Undifferentiated")
HKKtable2sed["HKK"][HKKtable2sed["HKK"] == "5"] <- "5"
HKKtable2sed["HKK"][HKKtable2sed["HKK"] == "10"] <- "10"
HKKtable2sed["HKK"][HKKtable2sed["HKK"] == "25"] <- "25"
HKKtable2sed["HKK"][HKKtable2sed["HKK"] == "100"] <- "100"
HKKtable2sed$extent<-factor(HKKtable2sed$HKK,levels=c("5","10","25","100"))

#palwes<-c("black","darkgrey","lightgrey")


sedplot<-ggplot(as.data.frame(HKKtable2sed),aes(x=as.factor(extent),y=Percent,fill=Diff_type,))+
  geom_bar(stat="identity")+
  scale_fill_manual(values=palwes)+
  xlab("Habitat Extent")+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90))+ 
  annotate("text", x = 1, y = 0.05, label = "6", size = 3.5)+
  annotate("text", x = 2, y = 0.05, label = "2", size = 3.5)+
  annotate("text", x = 3, y = 0.05, label = "21", size = 3.5)+
  annotate("text", x = 4, y = 0.05, label = "13", size = 3.5)+
  annotate("text", x = 1, y = 0.95, label = "8", size = 3.5,color="white")+
  annotate("text", x = 2, y = 0.95, label = "6", size = 3.5,color="white")+
  annotate("text", x = 3, y = 0.95, label = "9", size = 3.5,color="white")+
  annotate("text", x = 4, y = 0.95, label = "2", size = 3.5,color="white")+
  annotate("text", x = 1, y = 0.45, label = "9", size = 3.5)+
  annotate("text", x = 2, y = 0.45, label = "13", size = 3.5)+
  annotate("text", x = 3, y = 0.6, label = "10", size = 3.5)+
  annotate("text", x = 4, y = 0.8, label = "4", size = 3.5)+
  theme(legend.position = "none")+
   theme(panel.background = element_blank(), 
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
...
HKKtable2rocky<-pivot_longer(as.data.frame(HKKtablerocky), cols=1:4, names_to = "HKK",
                        values_to = "Percent", values_drop_na = FALSE)
HKKtable2rocky$Diff_type<-c("Statistical","Statistical","Statistical","Statistical","Pseudo-CS (Diagnostic)","Pseudo-CS (Diagnostic)","Pseudo-CS (Diagnostic)","Pseudo-CS (Diagnostic)","Undifferentiated","Undifferentiated","Undifferentiated","Undifferentiated")
HKKtable2rocky["HKK"][HKKtable2rocky["HKK"] == "5"] <- "5"
HKKtable2rocky["HKK"][HKKtable2rocky["HKK"] == "10"] <- "10"
HKKtable2rocky["HKK"][HKKtable2rocky["HKK"] == "25"] <- "25"
HKKtable2rocky["HKK"][HKKtable2rocky["HKK"] == "100"] <- "100"
HKKtable2rocky$extent<-factor(HKKtable2rocky$HKK,levels=c("5","10","25","100"))


rockyplot<-ggplot(as.data.frame(HKKtable2rocky),aes(x=as.factor(extent),y=Percent,fill=Diff_type,))+
  geom_bar(stat="identity")+
  scale_fill_manual(values=palwes)+
  xlab("Habitat Extent")+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90))+
  annotate("text", x = 1, y = 0.05, label = "6", size = 3.5)+
  annotate("text", x = 2, y = 0.05, label = "4", size = 3.5)+
  annotate("text", x = 3, y = 0.05, label = "6", size = 3.5)+
  annotate("text", x = 4, y = 0.25, label = "1", size = 3.5)+
  annotate("text", x = 1, y = 0.95, label = "21", size = 3.5,color="white")+
  annotate("text", x = 2, y = 0.95, label = "4", size = 3.5,color="white")+
  annotate("text", x = 3, y = 0.95, label = "18", size = 3.5,color="white")+
  annotate("text", x = 1, y = 0.4, label = "16", size = 3.5,color="white")+
  annotate("text", x = 2, y = 0.4, label = "9", size = 3.5)+
  annotate("text", x = 3, y = 0.4, label = "10", size = 3.5)+
  annotate("text", x = 4, y = 0.75, label = "1", size = 3.5)+
  theme(legend.position = "none")+
  theme(panel.background = element_blank(), 
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
...
ggarrange(
    ...
    ggarrange(sedplot, rockyplot, nrow = 2, labels = c("B", "C")), 
    ncol = 2, 
    labels = "A" , 
