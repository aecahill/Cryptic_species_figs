...
# Note Oct 4 Abby:
# Current version of the class file DOES CONTAIN Copepods/Thecostraca
...
write.csv(MissPerZone, file="Missed biological species per zone NCBI TRUE Oct4.csv")
...
write.csv(info, file="Missed biological species per class Oct4.csv")
...
genNpol <-unique(filter(survey,Npol==TRUE)[,c("genus","gs")])
mean(genNpol$gs)  # 35.94118 OCT 4!
...
mean(na.omit(genNtemp$gs)) # 32.09829 OCT 4!
...
mean(na.omit(genTrop$gs))  # 31.70115 OCT 4!
genStemp <-unique(filter(survey,Stemp==T)[,c("genus","gs")])
mean(na.omit(genStemp$gs)) # 31.02083 OCT 4!
genSpol <-unique(filter(survey,Spol==T)[,c("genus","gs")])
mean(na.omit(genSpol$gs))  # 26.94444 OCT 4!  ### GENRES PLUS PETITS  POUR LES COMPX SPP CRYPTIQUES DU POLE SUD

# genus size / zone latitude for ANY NOMINAL spp (incl. non cryptic):
genNpol <-unique(filter(nomsp,Npol==T)[,c("genus","gs")])
mean(genNpol$gs)  # 22.21615 OCT 4!
genNtemp <-unique(filter(nomsp,Ntemp==T)[,c("genus","gs")])
mean(genNtemp$gs) # 11.4279 OCT 4!
genTrop<-unique(filter(nomsp, Trop==T)[,c("genus","gs")])
mean(genTrop$gs)  # 11.19951 OCT 4!
genStemp<-unique(filter(nomsp, Stemp==T)[,c("genus","gs")])
mean(genStemp$gs) # 13.01794 OCT 4!
genSpol<-unique(filter(nomsp, Spol==T)[,c("genus","gs")])
mean(genSpol$gs)  # 21.02245 OCT 4! 
#Donc pour toutes les NS, genres plus grands au poles, pattern different qu'avec les CS (note Abby Oct 4)
...
write.csv(tab_phyla,file="Joint_Table_19phyla_977NS_Oct4.csv",row.names=T) 
...
f    
 write.csv(f,file="977NS_Fisher_chi_p_values_and_HKKv3Oct4.csv")
...
## (2) With distinct habitats: to see relationships between geo_dif (or eco_dif or morpho_dif) with habitats  did not use all habitat categories in this round below (see other loop below)
 ...
 f    
 write.csv(f,file="977NS_Fisher_chi_p_values_HabitatsOct4.csv")
...
# ecological differentiation (COMMENTAIRES D'ANNE, VIEUX)
...
da <-filter(survey, is.na(CHull_r100)==F)  #423 cases

#Below I removed 8 cases with 29 or 30 CS/cpx seemes outliers and impeeded to see well the other cases with reasonable Nbs of CS
...
hist(as.numeric(da$Nsg), breaks=100)

da5<-filter(da, as.numeric(Nsg)>4)  # 155 cases with at least 5 sites analysed in genetics
da6<-filter(da, as.numeric(Nsg)>5)  # 135 cases .... 6 ....
da7<-filter(da, as.numeric(Nsg)>6)  # 124 cases .... 7 ....
da8<-filter(da, as.numeric(Nsg)>7)  # 108 cases .... 8 ....
da9<-filter(da, as.numeric(Nsg)>8)  # 100 cases .... 9 ....
da10<-filter(da, as.numeric(Nsg)>9)  # 90 cases .... 10....

table(da$Geo_dif3)           
table(da10$Geo_dif3)         
table(da8$Geo_dif3)          
table(da7$Geo_dif3)          
tapply(da8$CHull_r100, da8$Geo_dif3, mean) 
tapply(da7$CHull_r100, da7$Geo_dif3, mean) 

table(da$Morpho_diff)        
table(da$Morpho_diff,da$Geo_dif3)

plot(as.numeric(da$Nig),as.numeric(da$Nsg))
summary(as.numeric(da$Nig))
summary(as.numeric(da5$Nig))

# PLOTS range size/ sympatry allopatry / Nb_CS...

# da$allop <-str_detect(da$Geo_diff,"Allopatric")
ggplot(da,aes(x=Nb_CS,y=CHull_r100,colour=Geo_dif3, size=as.numeric(Nsg)))+geom_point()+geom_smooth(method=lm,se=T, fullrange=TRUE)
ggplot(da,aes(x=Nb_CS,y=CHull_r100,colour=Geo_dif3, size=as.numeric(Nsg)))+geom_point()+geom_smooth(method=lm,se=F, fullrange=TRUE)
tapply(da$CHull_r100/1000000,da$Geo_dif3,mean_se)
...
ggplot(da,aes(x=as.numeric(Nsg),y=Nb_CS,colour=Geo_dif3))+geom_point()+geom_smooth(method=lm,se=F, fullrange=TRUE)
...
# 1- STUDY EFFORT
glm<-glm(Nb_CS~as.numeric(Nsg), data=survey, family="poisson")
summary(glm)
drop1(glm, test="Chisq") # *** !
mean(survey$Nb_CS/as.numeric(survey$Nsg), na.rm=T) # 0.6 CS / site genetically characterized in average but in regression model
glm<-glm(Nb_CS~as.numeric(Nsg)+0, data=survey, family="poisson")
...
glm1<-glm(Nb_CS~phylum_worms+as.numeric(Nsg), data=survey, family="poisson")
summary(glm1)
