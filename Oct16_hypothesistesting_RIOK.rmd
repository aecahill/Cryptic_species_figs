...
survey<-filter(survey, RIOK==TRUE)
...
This is NOW ONLY MARGINALLY significant. HKK=10, = 1000 trends DOWN, HKK=25, =100 trend UP. HKK = 5 doesn't trend but is largest before Darwin. So, it's messy.
...
Hypothesis not supported. DIFFERENT FROM ALL SPECIES DATA.

## In sum, when we only look at species with CS (i.e. our database), the effect of taxonomic work goes away. This is interesting since that was the strongest effect in paper 1. THIS IS STILL TRUE WITH RIOK=TRUE.
...
Hypothesis not supported. THIS IS DIFFERENT FROM USING ALL SPECIES.
...
Result: This is VERY different from the whole species dataset - marginally signficant. However, the difference is OPPOSITE from what was expected = higher NBCS in low HKK bins. Maybe compare pairwise in the future?

## In sum, NO support for the role of recent divergence as measured by a proxy of dispersal when considering only RIOK=TRUE cases.
...
Result: More CSss found in stable environments. Hypothesis supported. Stronger support with RIOK=TRUE.
...
Note3: Only 248 cases are considered here, meaning that is where we have morphological data AND larval type data AND RIOK = TRUE
...
Result: we do see more diagnostic differences in sympatry than allopatry; hypothesis supported. Lower p-value when RIOK=TRUE. 
...
THIS IS NO LONGER SUPPORTED, AND REALLY DRAMATICALLY SO, WHEN RIOK=TRUE. This must mean that species that were dropped in some way affected this. Investigate!
...
Result: more diagnostic traits with external, hypothesis accepted. This is also true if only considering CSss and CSsl (though p = 0.052, so marginal), AND when gastropods are removed (the only class with "both").

## In sum, fertilization and genitalia still have a role in determining the TYPE of CS that is found (ss vs sl) BUT the role of hard skeletons went away? Need to look into this.

## So, overall conclusions: When looking at the full dataset, there is support for several (but not all) sets of hypotheses. Interestingly, the effect of taxonomic work/effort seems to go away, while the effect of taxonomic class remains. Some support for both selection-based and neutral ideas. Slightly less support for biological factors here - both skeletons and larval types flipped significance dramatically when only considering RIOK. 