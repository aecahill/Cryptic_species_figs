...
survey<-filter(survey, phylum_wormsV1=="Echinodermata")
surveylarv<-filter(surveylarv, phylum_wormsV1=="Echinodermata")
...
More CSss than the 50% benchmark (but not significant), so reject the idea that most CS are just due to taxonomic oversight.
...
decadetable$decade<-c("1750","1770","1780","1800","1820","1830","1840",
                      "1860","1870","1890","1970")
...
There is no trend in the percent of CSsl (diagnostic differences) through time; hypothesis not supported. Note VERY small sample size.
...
There's marginal support here. Note that only one species is in the "modern" bin. So there's more HKK=5 and =10 in the early bin, and more HKK = 100 in the later one. Again, tiny sample sizes though.
...
decadetable$decade<-c("1750","1770","1780","1800","1810","1820","1830","1840",
                      "1850","1860","1870","1880","1890","1900","1910","1970","2000")
...
So the result is not significant but the graph is very much in line with our predictions - easy species mostly described early. 

## In sum, when we only look at species with CS (i.e. our database), the effect of taxonomic work goes away. This is interesting since that was the strongest effect in paper 1. Seems to still be true with the echinoderms, and again note very very small sample sizes - only 17 species have both a date of description and morpho data.

## Effect of recent divergence - dispersal
# Larval type -- Planktotrophic/pelagic larvae leads to less CS/NS)?
Note1: for this I'm working with a separate surveylarv input file where I have corrected larval types to pt, lt, dd, and removed all others, including cases where there are multiple types checked. (Most of those cases are where the two CS have different larvae.) This left 45 CS with larval info.
...
## In sum, NO support for the role of recent divergence as measured by a proxy of dispersal when considering only echinoderms. 
...
# Note also that stabilizing selection predicts more CSss than CSsl, which is not what we find (see above; find no difference between groups). 

## In sum, we have no support for this idea within the echinoderms.
...
Note3: Only 19 cases are considered here, meaning that is where we have morphological data AND larval type data AND echinoderms! Small sample size!
...
```
Result: Hypothesis not supported. We do see more diagnostic differences at low RI but the sample sizes are very low and the result is not significant.
...
#Graph
HKKtable2<-pivot_longer(as.data.frame(HKKtable), cols=1:4, names_to = "HKK",
                        values_to = "Percent", values_drop_na = FALSE)
HKKtable2$Diff_type<-c("Statistical","Statistical","Statistical","Statistical","Diagnostic","Diagnostic","Diagnostic","Diagnostic","No_Difference","No_Difference","No_Difference","No_Difference")
...
#Graph
HKK_NbCS<-pivot_longer(as.data.frame(HKKtable3), cols=1:4, names_to = "HKK",
                        values_to = "Percent", values_drop_na = FALSE)
HKK_NbCS$Diff_type<-c("CSss","CSss","CSss","CSss","CSsl","CSsl","CSsl","CSsl")
...
Results: Not longer significant, either with 3 morpho types OR when testing CSss vs sl. Only working with 25 cases.

## In sum, we see NO support for the neutral hypothesis with either reproductive investment and HKK.
...
Result: Not supported. Very small sample size here, just 20 cases.
...
No test; everyone has hard skeleton.
...
```
No test; no one has image vision.
...
Result: No test; everyone has external fertilization.
...
Result: No test; no one has genitalia.

## So, no tests of biological traits are possible here because they are fixed within the class.
