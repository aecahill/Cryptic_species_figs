...
#NEW FOR THIS SCRIPT: MAKE Nb CS complex per article = 1
# 609 cases remaining
...
## In sum, when we only look at species with CS (i.e. our database), the effect of taxonomic work goes away. This is interesting since that was the strongest effect in paper 1. THIS IS STILL TRUE WITH CS per article = 1.

## Effect of recent divergence - dispersal
# Larval type -- Planktotrophic/pelagic larvae leads to less CS/NS)?
Note1: for this I'm working with a separate surveylarv input file where I have corrected larval types to pt, lt, dd, and removed all others, including cases where there are multiple types checked. (Most of those cases are where the two CS have different larvae.) This left 410 CS with larval info.
...
Hypothesis supported, with DD having higher mean CS per nominal species. This is the same as all species and different from RIOK=TRUE. 
...
Result: More CSss found in stable environments, though p-value is marginal with this subset of data. Hypothesis supported. Weaker support than with whole dataset OR with RIOK=TRUE.
...
Results: More diagnostic differences (=CSsl) at lower HKK values, both when considering 3 morpho classes AND just ss vs sl. Overall p-value still significant but this pattern seems less clear. Lots more CSss at high HKK.
...
These results aren't statistically significant (ANCOVA) but the graph is in line with the hypothesis. Marginal support. HOWEVER these lines look closer together than the whole dataset or RIOK=true? Careful on interpretation.
...
Result: There are no longer significant associations here = different from whole dataset AND from RIOK=TRUE.
...
THIS IS NO LONGER SUPPORTED WHEN NbCS/article = 1. This must mean that species that were dropped in some way affected this. Investigate!
...
## When considering NbCS per article = 1, patterns are mostly similar to the full dataset. There is slightly less support for biological factors here (skeletons) AND we no longer see more diagnostic differences in sympatry.