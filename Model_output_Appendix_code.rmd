...
6) Is habitat stability associated with CSss? What about a model that includes both stability and connectivity (n = 492)
```{r}
Hstable<-c()
...
7) Are CSss more common in rocky or soft sediment habitats? (n = 227) What about when only considering shallow-water species? (n = 193)
```{r}
...
8) Are CSss more common in sympatry or no? (n = 486)
```{r}
summary(glmer(CSss~Sympatric+(1|phylum_class),data=surveymorpho,family=binomial))
```

9) Are species in sympatry more likely to have ecological differentiation? (n=978)
```{r}
summary(glmer(Eco_diff~Sympatric+(1|phylum_class),data=survey,family=binomial))

```

10) Are CSss more common in species with ecological differentiation? (n = 107)
```{r}
summary(glmer(CSss~Eco_diff+(1|phylum_class),data=surveymorpho,family=binomial))

```

11) Are biological traits (hard skeleton, fertilization type, presence of external genitalia, image-forming vision) associated with CSss? (n = 464)
```{r}
...
12) Is larval type associated with CSss? (n = 292)
```{r}
...
13) Are CSss more common in particular geographic zones? (n = 367)
```{r}
...
6) Is habitat stability associated with CSss? What about a model that includes both stability and connectivity (n = 411)
```{r}
Hstable<-c()
...
7) Are CSss more common in rocky or soft sediment habitats? (n = 190) What about when only considering shallow-water species? (n = 190)
```{r}
surveysed<-filter(surveymorpho, Hsubstrate != "NA") #make matrix where only the cases for rocky or sediment habitat
surveysed<-filter(surveysed, Hsubstrate != "Sediment and Rocky") #and remove the cases with both
...
8) Are CSss more common in sympatry or no? (n = 407)
```{r}
summary(glmer(CSss~Sympatric+(1|phylum_class),data=surveymorpho,family=binomial))
```

9) Are species in sympatry more likely to have ecological differentiation? (n=140)
```{r}
summary(glmer(Eco_diff~Sympatric+(1|phylum_class),data=survey,family=binomial))

```

10) Are CSss more common in species with ecological differentiation? (n = 104)
```{r}
summary(glmer(CSss~Eco_diff+(1|phylum_class),data=surveymorpho,family=binomial))

```

11) Are biological traits (hard skeleton, fertilization type, presence of external genitalia, image-forming vision) associated with CSss? (n = 386)
```{r}
summary(glmer(CSss~hard_skeleton+(1|phylum_class),data=surveymorpho,family=binomial))
summary(glmer(CSss~ferti+(1|phylum_class),data=surveymorpho,family=binomial))
summary(glmer(CSss~genitals+(1|phylum_class),data=surveymorpho,family=binomial))
...
12) Is larval type associated with CSss? (n = 246)
```{r}
surveylarvmorpho<-filter(surveylarv, Morpho_diff != "NA")
surveylarvmorpho<-filter(surveylarvmorpho, Larv_type != "NA")#make matrix where only the cases where morpho was studied is here
...
13) Are CSss more common in particular geographic zones? (n = 312)
```{r}
tropmorpho<-filter(surveymorpho, Trop != "NA") #make matrix where only the cases where morpho was studied is here
summary(glmer(CSss~Trop+(1|phylum_class),data=tropmorpho,family=binomial))
...
6) Is habitat stability associated with CSss? What about a model that includes both stability and connectivity (n = 395)
```{r}
Hstable<-c()
...
7) Are CSss more common in rocky or soft sediment habitats? (n = 171) What about when only considering shallow-water species? (n = 171)
```{r}
surveysed<-filter(surveymorpho, Hsubstrate != "NA") #make matrix where only the cases for rocky or sediment habitat
surveysed<-filter(surveysed, Hsubstrate != "Sediment and Rocky") #and remove the cases with both
...
8) Are CSss more common in sympatry or no? (n = 390)
```{r}
summary(glmer(CSss~Sympatric+(1|phylum_class),data=surveymorpho,family=binomial))
```

9) Are species in sympatry more likely to have ecological differentiation? (n=125)
```{r}
summary(glmer(Eco_diff~Sympatric+(1|phylum_class),data=survey,family=binomial))

```

10) Are CSss more common in species with ecological differentiation? (n = 91)
```{r}
summary(glmer(CSss~Eco_diff+(1|phylum_class),data=surveymorpho,family=binomial))

```

11) Are biological traits (hard skeleton, fertilization type, presence of external genitalia, image-forming vision) associated with CSss? (n = 368)
```{r}
summary(glmer(CSss~hard_skeleton+(1|phylum_class),data=surveymorpho,family=binomial))
summary(glmer(CSss~ferti+(1|phylum_class),data=surveymorpho,family=binomial))
summary(glmer(CSss~genitals+(1|phylum_class),data=surveymorpho,family=binomial))
...
12) Is larval type associated with CSss? (n = 261)
```{r}
surveylarvmorpho<-filter(surveylarv, Morpho_diff != "NA")
surveylarvmorpho<-filter(surveylarvmorpho, Larv_type != "NA")#make matrix where only the cases where morpho was studied is here
...
13) Are CSss more common in particular geographic zones? (n = 294)
```{r}
tropmorpho<-filter(surveymorpho, Trop != "NA") #make matrix where only the cases where morpho was studied is here
summary(glmer(CSss~Trop+(1|phylum_class),data=tropmorpho,family=binomial))
...
6) Is habitat stability associated with CSss? What about a model that includes both stability and connectivity (n = 383)
```{r}
Hstable<-c()
...
7) Are CSss more common in rocky or soft sediment habitats? (n = 160) What about when only considering shallow-water species? (n = 160)
```{r}
surveysed<-filter(surveymorpho, Hsubstrate != "NA") #make matrix where only the cases for rocky or sediment habitat
surveysed<-filter(surveysed, Hsubstrate != "Sediment and Rocky") #and remove the cases with both
...
8) Are CSss more common in sympatry or no? (n = 379)
```{r}
summary(glmer(CSss~Sympatric+(1|phylum_class),data=surveymorpho,family=binomial))
```

9) Are species in sympatry more likely to have ecological differentiation? (n=114)
```{r}
summary(glmer(Eco_diff~Sympatric+(1|phylum_class),data=survey,family=binomial))

```

10) Are CSss more common in species with ecological differentiation? (n = 81)
```{r}
summary(glmer(CSss~Eco_diff+(1|phylum_class),data=surveymorpho,family=binomial))

```

11) Are biological traits (hard skeleton, fertilization type, presence of external genitalia, image-forming vision) associated with CSss? (n = 361)
```{r}
summary(glmer(CSss~hard_skeleton+(1|phylum_class),data=surveymorpho,family=binomial))
summary(glmer(CSss~ferti+(1|phylum_class),data=surveymorpho,family=binomial))
summary(glmer(CSss~genitals+(1|phylum_class),data=surveymorpho,family=binomial))
...
12) Is larval type associated with CSss? (n = 236)
```{r}
surveylarvmorpho<-filter(surveylarv, Morpho_diff != "NA")
surveylarvmorpho<-filter(surveylarvmorpho, Larv_type != "NA")#make matrix where only the cases where morpho was studied is here
...
13) Are CSss more common in particular geographic zones? (n = 289)
```{r}
tropmorpho<-filter(surveymorpho, Trop != "NA") #make matrix where only the cases where morpho was studied is here
summary(glmer(CSss~Trop+(1|phylum_class),data=tropmorpho,family=binomial))
