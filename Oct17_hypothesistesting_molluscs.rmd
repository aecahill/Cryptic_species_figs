...
survey<-filter(survey, phylum_wormsV1=="Mollusca")
surveylarv<-filter(surveylarv, phylum_wormsV1=="Mollusca")
...
More CSss than the 50% benchmark (significant), so reject the idea that most CS are just due to taxonomic oversight. (This is closer to 0.05 than other subsets!)
...
This is NOW significant. HKK=5, = 100 trends DOWN as expected, and 100 trends UP, but 1000 also trends down. A bit messy.
...
Hypothesis maybe supported? The overall result is significant adn the graph is trending in the right direction, sort of.

## In sum, when we only look at species with CS (i.e. our database), the effect of taxonomic work goes away. This is interesting since that was the strongest effect in paper 1. THIS IS STILL TRUE with the molluscs.

## Effect of recent divergence - dispersal
# Larval type -- Planktotrophic/pelagic larvae leads to less CS/NS)?
Note1: for this I'm working with a separate surveylarv input file where I have corrected larval types to pt, lt, dd, and removed all others, including cases where there are multiple types checked. (Most of those cases are where the two CS have different larvae.) This left 112 CS with larval info.
...
## In sum, NO support for the role of recent divergence as measured by a proxy of dispersal when considering only the molluscs.
...
Result: More CSss found in stable environments, but not significant. Trends in direction expected.
...
Note3: Only 54 cases are considered here, meaning that is where we have morphological data AND larval type data AND Mollusc
...
Results: More diagnostic differences (=CSsl) at lower HKK values, both when considering 3 morpho classes AND just ss vs sl; lots of no difference at high HKK. Numbers are very small at 1 and 1000; to be ignored. Overall, this is decent support for the hypothesis. 
...
```
Result: more diagnostic differences with image vision; hypothesis supported. Might this also be confounded with class somehow?
...
Result: hypothesis not really supported. The gastropods are the largest thing in this dataset and are weighing heavily, with "both" and more diagnostic differences. Perhaps investigate further.
...
Result: Same as above; results here are heavily tilted towards the gastropods.

## In sum, biological traits still have a role in determining the TYPE of CS that is found (ss vs sl) BUT this may be partially confounded by class.

## So, overall conclusions: When looking at just the molluscs, most patterns are similar to the whole dataset. Most notably, we now have support for image vision. Of note: more molluscs in the dataset than any other phylum.