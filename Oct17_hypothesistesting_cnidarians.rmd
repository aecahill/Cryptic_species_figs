...
survey<-filter(survey, phylum_wormsV1=="Cnidaria")
surveylarv<-filter(surveylarv, phylum_wormsV1=="Cnidaria")
...
Significantly more ss than sl; reject hypothesis.
...
decadetable$decade<-c("1750","1760","1770","1780","1790","1810","1820","1840",
                      "1860","1870","1880","1890","1900","1910","1950","1960","1990","2000","2010")
...
There is no trend here. Hypothesis not supported.
...
This is  significant but messy. Patterns not very clear.
...
Significant but no trend, no real support for hypothesis.

## In sum, when we only look at species with CS (i.e. our database), the effect of taxonomic work goes away. This is interesting since that was the strongest effect in paper 1. THIS IS STILL TRUE with the cnidarians.

## Effect of recent divergence - dispersal
# Larval type -- Planktotrophic/pelagic larvae leads to less CS/NS)?
Note1: for this I'm working with a separate surveylarv input file where I have corrected larval types to pt, lt, dd, and removed all others, including cases where there are multiple types checked. (Most of those cases are where the two CS have different larvae.) This left 75 CS with larval info.
...
No differences; hypothesis not supported.
...
Result: No differences; hypothesis not supported.

## In sum, no support for the role of recent divergence as measured by a proxy of dispersal when considering only the cnidarians
...
Note3: Only 30 cases are considered here, meaning that is where we have morphological data AND larval type data AND Cnidarian
...
Results: Not significant; no support. Most Cnidarians are either at n=10 (reefs) or n=1000 (jellies).

## In sum, we do not see support for hypotheses related to Ne. 
...
Not significant; no support. Also true if I just test CSss/sl.
...
```
Result: No image vision; no test.
...
Result: hypothesis not supported. Only ONE internal fertilizer. (Who is this?)
...
Result: Everyone lacks genitals; no test.

## In sum, no support for the role of biological traits within the cnidarians, but these traits are fixed in this group of species.

## So, overall conclusions: When looking at just the cnidarians, most hypotheses lacking support? Maybe something about taxonomic history. Thought: maybe there's just so many CS overall in the cnidarians that no other patterns can emerge? Everyone has CS?