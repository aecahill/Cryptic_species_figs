...
# ... [AND other keywords (corresponding to the traits, for instance, seeked to fill in a database)]
# ... and to write in a table the reference numbers for each species.
# if the species list f synonym is ready and has been used already, and 1wordspecies list done , can start at PART C.
...
# 2) " taxon.txt "          : The WoRMS (world register of marine species) database main file : I obtained it by registering and asking to the staff of WoRMS.
...
# Load the Worms database (I obtained it from Worms but it requires a particular request/registering, etc)
...
# spp$class <-worms$class[match(spp$scientificName,worms$scientificName)] # in the present case (Medit_spp.csv) the variable already exists
...
summary(spp$Nb_synonyms) # mean = 8 for these 65 species
hist(spp$Nb_synonyms, breaks=67) 

# Below are some checks (were done for very big lists, not all are useful in our small example case here) 
...
spp <-filter(spp, is.na(synonymList)==F) # This step is necessary if you did not run code replacing missing synonimLists by scientificNames 
...
spp <-filter(spp, is.na(synonymList)==F) # This step is necessary if you did not run code replacing missing synonimLists by scientificNames 
spp <-filter(spp, (synonymList)!="") # This step is necessary if you did not run code replacing missing synonimLists by scientificNames 
...
#############################################################################################################
### STEP B : (OUTSIDE R) FIND THE BIBLIOGRAPHICAL REFERENCES IN WoS : ####
################################################################################################################
...
#allreferences <-read_xls("savedrecs.xls") <-commentaire from Abi
colstokeep <- c("Authors","Publication Year","Article Title","Abstract","Source Title","Volume","Start Page","End Page")
# allreferences %>% select(all_of(colstokeep)) # pas de message erreur mais ne retire pas les colonnes !!! alors qu'indique 8 colonnes dans la console ??????
...
table(duplicated(allreferences[,c(1:3,5:8)])) # I removed abstract to save planet
